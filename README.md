...
建议去皮肤科挂号看一下，本文的核心价值是提供一个药物名称的列表，你可以用 AI 或搜索去了解每一个药物。    

## 处理痤疮的药物
1. 维 A 酸乳膏（有三种浓度，0.1%, 0.05%, 0.025%）
- 0.025%（低浓度）：适合新手建立耐受、敏感肌，或者仅用于抗衰老和改善轻微黑头粉刺。
- 0.05%（中浓度）：常规治疗寻常痤疮（痘痘）的标准浓度，也是大多数人能耐受的上限。
- 0.1%（高浓度）：猛药，刺激性极强，通常只用于严重的结节囊肿型痤疮，或者用在背部等皮肤较厚的区域，脸上极易烂脸。
1. 夫西地酸乳膏：夫西地酸属于外用抗生素，专门用来对付红肿、发炎、按压有痛感或有脓头的痘痘（以及毛囊炎），作用是杀菌消炎，只涂在红肿处即可。
1. 过氧苯甲酰凝胶：可以白天用
...
## 整体地图
涂抹类（外用）：维 A 酸、夫西地酸、阿达帕林、过氧苯甲酰等。   
口服类（内服）：
- 抗生素（多西环素/米诺环素）：短期杀菌。
- 异维 A 酸（泰尔丝）：长期控油，重度痤疮的“核武器”。

物理/光电治疗：
- 基础清理：针清、红蓝光。
- 化学剥脱：刷酸（果酸/水杨酸/复合酸）。
- 针对痘印/痘坑：点阵激光（剥脱/非剥脱）、光子嫩肤（去红痘印）、微针（导入药物修护）。
- 急救：封闭针。
